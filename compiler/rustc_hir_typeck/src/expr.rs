...
use rustc_middle::ty::{
    self, AdtKind, GenericArgsRef, InitAdtComponentInfo, Ty, TypeVisitableExt, Unnormalized,
};
...
impl<'a, 'tcx> FnCtxt<'a, 'tcx> {
    ...
    fn check_expr_kind(
        ...
        match expr.kind {
            ...
            ExprKind::InitStruct(qpath, fields, ref base_expr) => {
                self.check_expr_init_struct(expr, expected, qpath, fields, base_expr)
            }
    ...
    fn check_expr_ptr_metadata(
        ...
        if let hir::StructTailExpr::DefaultFields(span) = *base_expr {
            ...
            let fru_tys = expected_fields
                .iter()
                .map(|(_, _, _, ty)| self.normalize(span, Unnormalized::new_wip(ty)))
                .collect();
            ...
            let fru_tys = expected_fields
                .iter()
                .map(|(_, _, _, ty)| self.normalize(expr.span, Unnormalized::new_wip(ty)))
                .collect();
            self.typeck_results.borrow_mut().fru_field_types_mut().insert(expr.hir_id, fru_tys);
                    ...
                    let (elem_args, elem_arg_tys): (Vec<_>, Vec<_>) = init_info
                        ...
                        .map(|arg| {
                            ...
                            if let Some(&(refd_field_idx, refd_field)) = adt_fields_by_name.get(&ident) {
                                let field_ty = 
self.normalize(expr.span, Unnormalized::new_wip(refd_field.ty(self.tcx, adt_args)));
            ...
            hir::StructTailExpr::DefaultFields(span) => {
                ...
                let fru_tys = match adt_ty.kind() {
                    ...
                    ty::Adt(adt, args) if adt.is_enum() => variant
                        ...
                        .collect(),
                    _ => {
                        let guar = self.dcx().emit_err(FunctionalRecordUpdateOnNonStruct { span });
                        return Ty::new_error(tcx, guar);
                    }
                };
                self.typeck_results.borrow_mut().fru_field_types_mut().insert(expr.hir_id, fru_tys);
            ...
            hir::StructTailExpr::Base(base_expr) => {
                ...
                let fru_tys = match adt_ty.kind() {
                    ty::Adt(adt, args) if adt.is_struct() => variant
                        ...
                        .map(|f| {
                            let field_ty = self
                                .normalize(expr.span, Unnormalized::new_wip(f.ty(self.tcx, args)));
