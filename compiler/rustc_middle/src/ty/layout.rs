                ...
                ty::Ref(_, pointee, _) | ty::RawPtr(pointee, _) => {
                        ...
                        && !pointee.references_error()
                    {
                        ...
                        match tail.kind() {
                            ...
                            ty::Dynamic(_, _, ty::Dyn) => {
                                let dyn_metadata =
                                    tcx.require_lang_item(LangItem::DynMetadata, DUMMY_SP);
                                tcx.type_of(dyn_metadata).instantiate(tcx, &[tail.into()])
                            }
    ...
    fn ty_and_layout_pointee_info_at(
        ...
        let pointee_info = match *this.ty.kind() {
            ...
            _ => {
                ...
                {
                    let optimize = tcx.sess.opts.optimize != OptLevel::No;
                    pointee.safe = Some(PointerKind::SharedRef { frozen: true });
                    pointee.align = Primitive::Pointer(AddressSpace::ZERO).align(cx).abi;
                    let min_count = if optimize {
                    ...
                    pointee.size = Primitive::Pointer(AddressSpace::ZERO)
                        .size(cx)
