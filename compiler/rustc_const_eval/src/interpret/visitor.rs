        ...
        match *ty.kind() {
            ...
            ty::Adt(def, ..) if def.is_box() => {
                ...
                let [unique_ptr, alloc] =
                    self.ecx().project_simple_fields(v, [FieldIdx::ZERO, FieldIdx::ONE])?;

                // Unfortunately there is some type junk in the way here: `unique_ptr` is a `Unique`...
                // (which means another 2 fields, the second of which is a `PhantomData`)
                assert_eq!(unique_ptr.layout().fields.count(), 2);
                let [nonnull_ptr, phantom] = self
                    .ecx()
                    .project_simple_fields(&unique_ptr, [FieldIdx::ZERO, FieldIdx::ONE])?;
                assert!(
                ...
                let raw_ptr = self.ecx().project_simple_field(&nonnull_ptr, FieldIdx::ZERO)?; // the actual raw ptr
        ...
        match &v.layout().fields {
            ...
            FieldsShape::Arbitrary { in_memory_order, .. } => {
                for idx in in_memory_order.iter().copied() {
                    let field = self.ecx().project_simple_field(v, idx)?;
