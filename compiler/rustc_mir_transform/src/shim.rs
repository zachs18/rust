...
use rustc_middle::ty::{
    ...
    InitAdtComponentInfo, Ty, TyCtxt, Unnormalized,
};
...
impl<'tcx> LayoutForMetaShimBuilder<'tcx> {
    fn new(
        ...
        let sig = tcx.fn_sig(method_def_id).instantiate(tcx, &[self_ty.into()]).skip_norm_wip();
        let sig = tcx.instantiate_bound_regions_with_erased(sig);
...
impl<'tcx> PtrMetadataCmpShimBuilder<'tcx> {
    fn new(
        ...
        let sig = tcx
            .fn_sig(def_id)
            .instantiate(tcx, &[Ty::new_ptr_metadata(tcx, pointee_ty).into()])
            .skip_norm_wip();
        let sig = tcx.instantiate_bound_regions_with_erased(sig);
        let span = tcx.def_span(def_id);
