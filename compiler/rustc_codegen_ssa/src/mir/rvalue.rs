...
impl<'a, 'tcx, Bx: BuilderMethods<'a, 'tcx>> FunctionCx<'a, 'tcx, Bx> {
    ...
    pub(crate) fn codegen_coerce_unsized_into_operand(
        ...
        match (src_ty.kind(), dst_ty.kind()) {
            (&ty::Pat(_s, sp), &ty::Pat(_d, dp))
                if let (PatternKind::NotNull, PatternKind::NotNull) = (*sp, *dp) =>
            {
                let mut dst = OperandRefBuilder::new(dst_layout);

                let src_f = src.extract_field(self, bx, 0);
                let dst_f_layout = dst_layout.field(bx, 0);
                ...
                dst.insert_field(bx, FIRST_VARIANT, FieldIdx::ZERO, dst_f_op);
