...
[ptr-metadata-type]: #ptr-metadata-type
[fn-type]: #fn-type
...
* `H` — A pointer metadata `core::ptr::Metadata<T>` / `builtin # ptr_metadata(T)`.

  > <span id="ptr-metadata-type">ptr-metadata-type</span> → `H` *[type]*

  The tag `H` is followed by the *[type]* of the pointer metadata.
