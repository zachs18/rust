...
use core::marker::{Freeze, PointeeSized};
...
fn ptr_metadata() {
    ...
    fn assert_thin<T: PointeeSized + ptr::Thin>(p: *const T) {
        let meta = metadata(p);
        assert_eq!(size_of_val(&meta), 0);
        assert_eq!(align_of_val(&meta), 1);
    }
    let () = assert_thin(&());
    let () = assert_thin(&Unit);
    let () = assert_thin(&4_u32);
    let () = assert_thin(&String::new());
    let () = assert_thin(&Some(4_u32));
    let () = assert_thin(&ptr_metadata);
    let () = assert_thin(&|| {});
    let () = assert_thin(&[4, 7]);
    let () = assert_thin(&(4, String::new()));
    let () = assert_thin(&Pair(4, String::new()));
    let () = assert_thin(ptr::null::<()>() as *const Extern);
    let () = assert_thin(ptr::null::<()>() as *const <&u32 as std::ops::Deref>::Target);

    assert_eq!(metadata("foo").len, 3_usize);
    assert_eq!(metadata(&[4, 7][..]).len, 2_usize);

    let dst_struct: &Pair<bool, [u8]> = &Pair(true, [0x66, 0x6F, 0x6F]);
    assert_eq!(metadata(dst_struct).1.len, 3_usize);
    unsafe {
    ...
    let vtable_1: DynMetadata<dyn Debug> = metadata(&4_u16 as &dyn Debug).vtable;
    let vtable_2: DynMetadata<dyn Display> = metadata(&4_u16 as &dyn Display).vtable;
    let vtable_3: DynMetadata<dyn Display> = metadata(&4_u32 as &dyn Display).vtable;
    let vtable_4: DynMetadata<dyn Display> =
        metadata(&Pair(true, 7_u32) as &Pair<bool, dyn Display>).1.vtable;
    unsafe {
...
fn ptr_metadata_bounds() {
    ...
    fn metadata_eq_method_address<T: ?Sized>() -> usize {
        // The `Metadata` type always implements `Ord`, so this is valid:
        <Metadata<T> as PartialEq>::eq as usize
    }
    // FIXME(ptr_metadata_v2): update this comment
    ...
    let _ = static_assert_expected_bounds_for_metadata::<Metadata<[u32; 5]>>;
    let _ = static_assert_expected_bounds_for_metadata::<Metadata<[u32]>>;
    let _ = static_assert_expected_bounds_for_metadata::<Metadata<dyn Display>>;
    fn _static_assert_associated_type<T: ?Sized>() {
        let _ = static_assert_expected_bounds_for_metadata::<Metadata<T>>;
    }
...
fn pointee_metadata_debug() {
    assert_eq!("Metadata", format!("{:?}", metadata::<u32>(&17)));
    assert_eq!("Metadata { len: 2, .. }", format!("{:?}", metadata::<[u32]>(&[19, 23])));
    let for_dyn = format!("{:?}", metadata::<dyn Debug>(&29));
    assert!(for_dyn.starts_with("Metadata { vtable: DynMetadata(0x"), "{:?}", for_dyn);
}
...
fn dyn_metadata() {
    ...
    assert_eq!(meta.checked_size_of().unwrap(), 64);
    assert_eq!(meta.checked_size_of().unwrap(), size_of::<Something>());
    assert_eq!(meta.checked_align_of().unwrap(), 32);
    assert_eq!(meta.checked_align_of().unwrap(), align_of::<Something>());
    assert_eq!(meta.checked_layout().unwrap(), std::alloc::Layout::new::<Something>());

    assert!(format!("{meta:?}").starts_with("Metadata { vtable: DynMetadata(0x"));
}
...
fn from_raw_parts() {
    ...
    let array_meta: Metadata<[u32; 5]> = builtin!(ptr_metadata(..));
    assert_eq!(ptr::from_raw_parts(address, array_meta), array_ptr.as_ptr());
    assert_eq!(ptr::from_raw_parts_mut(address, array_meta), array_ptr.as_ptr());
    assert_eq!(NonNull::from_raw_parts(NonNull::new(address).unwrap(), array_meta), array_ptr);

    let slice_meta: Metadata<[u32]> = builtin!(ptr_metadata(len: 5, ..));
    assert_eq!(ptr::from_raw_parts(address, slice_meta), slice_ptr.as_ptr());
    assert_eq!(ptr::from_raw_parts_mut(address, slice_meta), slice_ptr.as_ptr());
    assert_eq!(NonNull::from_raw_parts(NonNull::new(address).unwrap(), slice_meta), slice_ptr);
}
...
fn thin_box() {
        ...
        T: ?Sized,
    {
        ...
        fn layout(&self) -> (Layout, usize) {
            let meta = self.meta();
            Layout::for_value(&meta).extend(meta.checked_layout().unwrap()).unwrap()
        }
...
fn test_ptr_metadata_in_const() {
    ...
    const ARRAY_META: Metadata<[u16; 3]> = std::ptr::metadata::<[u16; 3]>(&[1, 2, 3]);
    const SLICE_META: Metadata<[u16]> = std::ptr::metadata::<[u16]>(&[1, 2, 3]);
    const DYN_META: Metadata<dyn Debug> = std::ptr::metadata::<dyn Debug>(&[0_u8; 42]);
    assert_eq!(ARRAY_META.elem, std::ptr::metadata(&0u16));
    assert_eq!(SLICE_META.len, 3);
    assert_eq!(DYN_META.vtable.size_of(), 42);
}
