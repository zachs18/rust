    ...
    PinInit<[T], Error, Arg> for Chain<I1, I2>
{
    ...
    unsafe fn init(
        ...
        unsafe {
            I1::init(this.init1, dst1, arg.clone(), pre_zeroed)?;
        }
        ...
        unsafe {
            I2::init(this.init2, dst2, arg, pre_zeroed)?;
        }
...
unsafe impl<Error, Arg: Clone, I1: PinInit<str, Error, Arg>, I2: PinInit<str, Error, Arg>>
    PinInit<str, Error, Arg> for Chain<I1, I2>
{
    ...
    unsafe fn init(
        ...
        // SAFETY: `dst1` has the correct length for `init1`
        unsafe {
            PinInit::init(crate::init::as_bytes(this.init1), dst1, arg.clone(), pre_zeroed)?;
        }
        // Don't need a drop guard because `str` doesn't need dropping

        // SAFETY: `dst2` has the correct length for `init2`
        unsafe {
            PinInit::init(crate::init::as_bytes(this.init2), dst2, arg, pre_zeroed)?;
        }
        Ok(())
    }
}
unsafe impl<Error, Arg: Clone, I1: Init<str, Error, Arg>, I2: Init<str, Error, Arg>>
    Init<str, Error, Arg> for Chain<I1, I2>
{
}
