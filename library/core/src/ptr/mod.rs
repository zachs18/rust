...
pub use metadata::{
    DynMetadata, Metadata, Thin, build_metadata, from_raw_parts, from_raw_parts_mut, metadata,
};
...
pub const fn null<T: PointeeSized + Thin>() -> *const T {
    from_raw_parts(without_provenance::<()>(0), build_metadata!(..))
}
...
pub const fn null_mut<T: PointeeSized + Thin>() -> *mut T {
    from_raw_parts_mut(without_provenance_mut::<()>(0), build_metadata!(..))
}
